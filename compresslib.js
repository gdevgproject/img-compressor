 ...
 *
 * Features:
 * - Dual/Multi-version output with customizable presets
 * - Real-time progress streaming with Web Workers support
 * - AI-powered image analysis and optimization
 * - Animated GIF support with frame optimization
 * - TypeScript-ready with JSDoc annotations
 * - Framework agnostic (Vanilla JS, React, Next.js, Angular, Vue)
 ...
 *
 * const result = await compressor.compress(file, {
 *   presets: ['instagram', 'thumbnail']
 * });
 *
 *
 * // 5. REACT HOOK
 * import { UltraCompressPro } from './compresslib.js';
 *
 * function useImageCompression() {
 *   const [progress, setProgress] = useState(0);
 *   const [results, setResults] = useState([]);
 *   const compressor = useRef(new UltraCompressPro());
 *
 *   const compress = async (files) => {
 *     const result = await compressor.current.compressMultiple(files, {
 ...
 *   };
 *
 *   return { compress, progress, results };
 * }
 *
 *
 * // 6. NEXT.JS API Route
 * import { UltraCompressPro } from '@/lib/compresslib';
 *
 ...
 * import { UltraCompressPro } from './compresslib';
 *
 ...
 *   };
 *
 *   return { compress, progress, results, stats: () => compressor.getStats() };
 * }
 *
 *
 * // 9. ANALYZE IMAGE BEFORE COMPRESSION
 ...
 *
 *
 * // 11. CANCEL BATCH OPERATION
 * const batchPromise = compressor.compressMultiple(largeFileList);
 *
